...
func (s *Store) BatchInsertAC(records []*model.AccommodationCatering) error {
	...
	stmt, err := tx.Prepare(`
		...
		ON CONFLICT(data_year, data_month, credit_code, industry_type) DO UPDATE SET
			name = excluded.name,
			...
			revenue_last_year_month = excluded.revenue_last_year_month,
			revenue_month_rate = excluded.revenue_month_rate,
			revenue_prev_cumulative = excluded.revenue_prev_cumulative,
			revenue_current_cumulative = excluded.revenue_current_cumulative,
			revenue_last_year_cumulative = excluded.revenue_last_year_cumulative,
			revenue_cumulative_rate = excluded.revenue_cumulative_rate,
			room_prev_month = excluded.room_prev_month,
			room_current_month = excluded.room_current_month,
			room_last_year_month = excluded.room_last_year_month,
			room_prev_cumulative = excluded.room_prev_cumulative,
			room_current_cumulative = excluded.room_current_cumulative,
			room_last_year_cumulative = excluded.room_last_year_cumulative,
			food_prev_month = excluded.food_prev_month,
			food_current_month = excluded.food_current_month,
			food_last_year_month = excluded.food_last_year_month,
			food_prev_cumulative = excluded.food_prev_cumulative,
			food_current_cumulative = excluded.food_current_cumulative,
			food_last_year_cumulative = excluded.food_last_year_cumulative,
			goods_prev_month = excluded.goods_prev_month,
			goods_current_month = excluded.goods_current_month,
			goods_last_year_month = excluded.goods_last_year_month,
			goods_prev_cumulative = excluded.goods_prev_cumulative,
			goods_current_cumulative = excluded.goods_current_cumulative,
			goods_last_year_cumulative = excluded.goods_last_year_cumulative,
			retail_current_month = excluded.retail_current_month,
			retail_last_year_month = excluded.retail_last_year_month,
			is_small_micro = excluded.is_small_micro,
			is_eat_wear_use = excluded.is_eat_wear_use,
			first_report_ip = excluded.first_report_ip,
			fill_ip = excluded.fill_ip,
			network_sales = excluded.network_sales,
			opening_year = excluded.opening_year,
			opening_month = excluded.opening_month,
			original_revenue_current_month = excluded.original_revenue_current_month,
			original_room_current_month = excluded.original_room_current_month,
			original_food_current_month = excluded.original_food_current_month,
			original_goods_current_month = excluded.original_goods_current_month,
