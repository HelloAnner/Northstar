...
func (s *Store) BatchInsertWR(records []*model.WholesaleRetail) error {
	...
	stmt, err := tx.Prepare(`
		INSERT INTO wholesale_retail (
			...
			retail_prev_month, retail_current_month, retail_last_year_month, 零售业销售额增速_当月,
			retail_prev_cumulative, retail_last_year_prev_cumulative,
			retail_current_cumulative, retail_last_year_cumulative, 零售业销售额增速_累计,
			retail_ratio,
		...
		ON CONFLICT(data_year, data_month, credit_code, industry_type) DO UPDATE SET
			...
			sales_prev_month = excluded.sales_prev_month,
			sales_current_month = excluded.sales_current_month,
			sales_last_year_month = excluded.sales_last_year_month,
			sales_month_rate = excluded.sales_month_rate,
			sales_prev_cumulative = excluded.sales_prev_cumulative,
			sales_last_year_prev_cumulative = excluded.sales_last_year_prev_cumulative,
			sales_current_cumulative = excluded.sales_current_cumulative,
			sales_last_year_cumulative = excluded.sales_last_year_cumulative,
			sales_cumulative_rate = excluded.sales_cumulative_rate,
			retail_prev_month = excluded.retail_prev_month,
			retail_current_month = excluded.retail_current_month,
			retail_last_year_month = excluded.retail_last_year_month,
			零售业销售额增速_当月 = excluded.零售业销售额增速_当月,
			retail_prev_cumulative = excluded.retail_prev_cumulative,
			retail_last_year_prev_cumulative = excluded.retail_last_year_prev_cumulative,
			retail_current_cumulative = excluded.retail_current_cumulative,
			retail_last_year_cumulative = excluded.retail_last_year_cumulative,
			零售业销售额增速_累计 = excluded.零售业销售额增速_累计,
			retail_ratio = excluded.retail_ratio,
			cat_grain_oil_food = excluded.cat_grain_oil_food,
			cat_beverage = excluded.cat_beverage,
			cat_tobacco_liquor = excluded.cat_tobacco_liquor,
			cat_clothing = excluded.cat_clothing,
			cat_daily_use = excluded.cat_daily_use,
			cat_automobile = excluded.cat_automobile,
			...
			original_sales_current_month = excluded.original_sales_current_month,
			original_retail_current_month = excluded.original_retail_current_month,
